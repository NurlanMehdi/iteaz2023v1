...
@section('content')
    ...
    <section class="mainPage">
        <div class="container-lg">
            ...
            <form id="aboutUs" class="section_articleInner_body_content--description" enctype="multipart/form-data" method="post" action="{{route('admin.save-resource')}}">
                {{csrf_field()}}
                <div class="page_content">
                    <form class="section_articleInner_body_content--description">
                        ...
                        <div class="form-group required">
                            <label class="customLabel" for="work_hours">Kateqoriya</label>
                            <select class="customSelect" {{(($data->category ?? '') != '') ? 'readonly' : ''}} name="category" id="category" required="" title="Kateqoriya">
                                @if(($data->category ?? '')== 0)
                                    <option value="0" selected>Müəllimlər üçün</option>
                                @elseif(($data->category ?? '') == 1)
                                    <option value="1" selected>Şagirdlər üçün</option>
                                @elseif(($data->category ?? '') == 2)
                                    <option value="2" selected>Valideynlər üçün</option>
                                @else
                                    <option value="0">Müəllimlər üçün</option>
                                    <option value="1">Şagirdlər üçün</option>
                                    <option value="2">Valideynlər üçün</option>
